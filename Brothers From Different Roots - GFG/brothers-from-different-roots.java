...
class Solution
{
    ...
    public static int solve(Node root, HashSet<Integer> set,int x){
        if(root==null){
            return 0;
        }
        int t1 = 0;
        ...
        if(set.contains(x-root.data)){
            t1 = 1+solve(root.left,set,x)+solve(root.right,set,x);
        }else{
            t2 = solve(root.left,set,x)+solve(root.right,set,x);
        }
        return t1+t2;
    }
}
